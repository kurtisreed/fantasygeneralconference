...
/**
 * Handle a posted "sessions watched" self-report: store it, rescore, and
 * respond appropriately for either the auto-saving fetch request or a plain
 * form post (no JS). Shared by the player's own sheet and the standings
 * page's "view my sheet" panel, so a self-report from either place behaves
 * identically — including a session that hasn't started yet being rejected
 * here too, not just grayed out in the form.
 */
function handle_watched_post(array $event, array $sessions, array $questions, int $playerId, string $redirectTo): void
{
    $validCodes = array_diff(array_column($sessions, 'code'), future_session_codes($event, $sessions));
    $checked = array_values(array_intersect((array)($_POST['watched'] ?? []), $validCodes));
    $n = min(watched_max($questions), count($checked));
...
/**
 * The "Sessions watched" self-report card: pill checkboxes per session,
 * grayed out until a session's real-world start time passes. Shared by the
 * player's own sheet and the standings page's "view my sheet" panel.
 */
function render_watched_card(array $player, array $sessions, array $questions, array $futureCodes): void
{
    $watchedCodes = array_filter(explode(',', (string)($player['watched_sessions'] ?? ''))); ?>
...
/** Auto-save script for render_watched_card(): posts on every checkbox toggle. */
function render_watched_autosave_script(): void
{
    ?>
<script>
(function () {
